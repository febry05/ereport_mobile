...
<head>
    <meta charset="UTF-8" />
    <title>Daftar Klien - APS SUPPORT</title>
...
<body>
    ...
    <div class="container py-5">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
            ...
            <form method="GET" action="{{ route('klien.index') }}" class="d-flex gap-2 mt-2 mt-md-0">
                    ...
                    @forelse ($klien as $index => $item)
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="btn-group" role="group" aria-label="Aksi klien">
                                    <button
                                        class="btn btn-info btn-sm"
                                        data-bs-toggle="modal"
                                        data-bs-target="#detailModal"
                                        data-id="{{ $item->id_klien }}"
                                        data-nama="{{ $item->nama_klien }}"
                                        data-nip="{{ $item->nip }}"
                                        data-notlp="{{ $item->notlp_klien }}"
                                        data-alamat="{{ $item->alamat_klien }}"
        ...
        <div class="mt-3 d-flex justify-content-between align-items-center">
            ...
            class="btn btn-pagination btn-sm @if($klien->onFirstPage()) disabled @endif"
            @if($klien->onFirstPage()) tabindex="-1" aria-disabled="true" @endif>
            &larr; Prev
            </a>
            ...
            class="btn btn-pagination btn-sm @if(!$klien->hasMorePages()) disabled @endif"
            @if(!$klien->hasMorePages()) tabindex="-1" aria-disabled="true" @endif>
            Next &rarr;
            </a>
        </div>
    </div>
    ...
    <div class="modal fade" id="addModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="POST" action="{{ route('klien.store') }}" class="modal-content shadow-lg border-0 rounded-3">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nama_klien" class="form-control shadow-sm" value="{{ old('nama_klien') }}" required />
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="notlp_klien" class="form-control shadow-sm" value="{{ old('notlp_klien') }}" required />
                        @error('notlp_klien') <small class="text-danger">{{ $message }}</small> @enderror
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Alamat</label>
                        <textarea name="alamat_klien" class="form-control shadow-sm" required>{{ old('alamat_klien') }}</textarea>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="POST" id="editForm" class="modal-content shadow-lg border-0 rounded-3">
                ...
                <div class="modal-body">
                    <input type="hidden" name="id_klien" id="edit_id_klien" />
                    <div class="mb-3">
                        <label class="form-label">Nama Klien</label>
                        <input type="text" name="nama_klien" id="edit_nama_klien" class="form-control shadow-sm" required />
                    </div>
                    <div class="mb-3">
                        <label class="form-label">NIP</label>
                    ...
                    <div class="mb-3">
                        <label class="form-label">Nomor Telepon</label>
                        <input type="text" name="notlp_klien" id="edit_notlp_klien" class="form-control shadow-sm" required />
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Alamat klien</label>
                        <textarea name="alamat_klien" id="edit_alamat_klien" class="form-control shadow-sm" required></textarea>
                    </div>
                    <div>
                        <label class="form-label">Divisi</label>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <form method="POST" id="hapusForm" class="modal-content shadow-lg border-0 rounded-3">
                ...
                <div class="modal-body text-center">
                    <p class="mb-4">
                        Apakah Anda yakin ingin menghapus data <strong id="hapusNamaklien"></strong>?
                    </p>
    ...
    <script>
        ...
        editModal.addEventListener('show.bs.modal', event => {
            ...
            form.action = `/klien/${id}`;

            document.getElementById('edit_id_klien').value = id;
            document.getElementById('edit_nama_klien').value = nama;
            document.getElementById('edit_nip').value = nip;
            document.getElementById('edit_notlp_klien').value = notlp;
            document.getElementById('edit_alamat_klien').value = alamat;
        ...
        hapusModal.addEventListener('show.bs.modal', event => {
            ...
            document.getElementById('hapusNamaklien').textContent = nama;
        });
    </script>
</body>
</html>
